...
#  }
  ...
  exec { "start_etcd_Container" :    
    command => "sudo docker run -d -p 4001:4001 -v /usr/share/ca-certificates/:/etc/ssl/certs quay.io/coreos/etcd:v2.0.11 -advertise-client-urls http://192.168.42.77:4001 -listen-client-urls http://0.0.0.0:4001",
    cwd => "/",
    path => "/usr/local/bin:/bin:/usr/bin",
  }

  exec { "start_dockerui_Container" :    
    command => "sudo docker run -d -p 9000:9000 -v /var/run/docker.sock:/docker.sock dockerui/dockerui -e /docker.sock",
    cwd => "/",
    path => "/usr/local/bin:/bin:/usr/bin",
  }
